	...
	function getTabData() {
		...
		function printTabs(tabs) {
					...
					chrome.tabs.sendMessage(tabId, { action: 'getChannelName' }, response => {
						...
						<li style="list-style-type:none;">
							...
							if (subscriptionStatus == false) {
								tabList = `${tabList}<button class="subscribe-button" id="tab-subscribe-${tabId.toString()}" title="Subscribe to this channel">${numSubs}</button>`;
							} else {
								tabList = `${tabList}<button class="subscribed-button" id="tab-subscribed-${tabId.toString()}" title="Subscribed to this channel">${numSubs}</button>`;
							}
							...
							if (likeStatus == true) {
							tabList = `${tabList}<button class="liked-button" id="tab-button-liked-${tabId.toString()}" title="You like this video"></button>`;
							} else {
							tabList = `${tabList}<button class="like-button" id="tab-button-like-${tabId.toString()}" title="Like this video"></button>`;
							}

							tabList = `${tabList}<button class="watch-later-button" id="tab-button-watch-later-${tabId.toString()}" title="Add this video to watch later"></button>`;

							tabList = `${tabList}<button class="previous-button" id="tab-button-prev-${tabId.toString()}"><span class="prev-button-popup"><span class="popup-header">Previous:</span> ${prevVideo}</span></button>
							`;
							...
							tabList = `${tabList}
							<button class="next-button" id="tab-button-next-${tabId.toString()}"><span class="next-button-popup"><span class="popup-header">Next Up:</span> ${nextVideo}</span></button>
							`;
							...
							if (isMute == true) {
								...
								tabList = `${tabList}<input type="button" class="unmute-button" id="tab-button-unmute-${tabId.toString()}" title="Unmute this video">`;
							} else {
								// tabList = `${tabList}<button class="mute-button" id="tab-button-mute-${tabId.toString()}" title="Mute this video"></button>`;
								tabList = `${tabList}<input type="button" class="mute-button" id="tab-button-mute-${tabId.toString()}" title="Mute this video">`;
							}
							if (dislikeStatus == true) {
							tabList = `${tabList}<button class="disliked-button" id="tab-button-disliked-${tabId.toString()}" title="You dislike this video"></button>`
							} else {
							tabList = `${tabList}<button class="dislike-button" id="tab-button-dislike-${tabId.toString()}" title="Dislike this video"></button>`
							}
			...
			function setListeners(tab) {
				...
				console.log('setting listeners2');

				$(loopButtonId).click(function(){
					...
					// chrome.runtime.sendMessage({ tabId: id, buttonClicked: true, totalTimeSeconds, totalDuration });
				});

				$(loopedButtonId).click(function(){
					console.log('looped button clicked');
					let id = $(event.target).attr('id');
					id = parseInt(id.split('-')[id.split('-').length - 1]);
					// location.reload();
					// sendMessage(id, 'reloadPage');
					// localStorage.setItem(`looped-${tabId}`, false);
					// location.reload();
				...
				$(likeButtonId).click(function(){
					...
					// $(likeButtonId).fadeOut(500, function() {
					// 	$(likeButtonId).css('background', "url('" + newImageURL + "') no-repeat center");
					// 	$(likeButtonId).css('background-size', "25px auto");

					// 	$(likedButtonId).fadeIn(500);
					// })
				});
				$(likedButtonId).click(function(){
				...
				$(dislikeButtonId).click(function(){
				...
				$(dislikedButtonId).click(function(){
				...
				$(watchLaterButtonId).click(function(){
			...
			for (var i = 0; i < tabs.length; i++) {
				...
				populateData(tabs[i], setListeners);
